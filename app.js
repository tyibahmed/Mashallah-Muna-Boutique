...
function applyFilter(tab){
  state.activeTab = tab;
  state.filtered = tab === 'all' ? state.products : state.products.filter(p=>p.category===tab);
}
    ...
    return `
      <div class="card" data-id="${p.id}">
        ...
        <div class="card-body">
          ...
          <div class="actions">
            <button class="btn add" data-id="${p.id}" type="button">Add to cart</button>
          </div>
        </div>
      </div>
    `;
...
window.addEventListener('DOMContentLoaded', ()=>{
  const cartBtn = $('#openCart'); if(cartBtn) cartBtn.onclick = openCart;
});
